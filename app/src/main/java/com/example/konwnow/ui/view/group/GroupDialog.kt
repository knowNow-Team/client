...
class GroupDialog(context: Context,makeGroupInterface: MakeGroupInterface) : Dialog(context), View.OnClickListener{
    ...
    init{
        this.makeGroupInterface = makeGroupInterface
    }

    fun start() {
        dlg.requestWindowFeature(Window.FEATURE_NO_TITLE)   //타이틀바 제거
        dlg.setContentView(R.layout.dialog_make_groups)     //다이얼로그에 사용할 xml 파일을 불러옴
        dlg.setCancelable(false)    //다이얼로그의 바깥 화면을 눌렀을 때 다이얼로그가 닫히지 않도록 함

        (App.instance).dialogResize(dlg, 0.9f, 0.6f)
